...
后代选择器： $("div .c1")
子代选择器： $("div>span")
相邻兄弟选择器： $("h1+p")  匹配选择器1后的第一个兄弟元素,同时要求兄弟元素满足选择器2  必须是相邻的 不相邻的不行
...
:gt(index)
  匹配下标大于index的元素
:not(选择器)     li:not(:last) // 除了最后一个都选上 li:not(li:first)
  否定筛选,除()中选择器外,其他元素
```
#### 5）操作元素内容
```javascript
...
$('#btn').click(function(){
            // $('#show').text()=$('#text').val()  不能直接赋值，因为不再是属性，而是方法，所以使用text赋值要用传参数的方法
            $('#show').text($('#text').val())
        })
```
#### 6）操作标签属性
1. attr("attrName","value")
    设置或读取标签属性:只写一个参数attrName 就是读取标签属性
...
$obj.before(newObj);	//在$obj的前面添加兄弟元1素
```
4. 移除元素 
...
2. 通过使用滑动下拉和上推效果，显示隐藏的被选元素（ **只针对块元素** ）
   ...
   ```javascript
    slideDown(speed,callback)/slideUp(speed,callback)  slideDown 是出现
   ```

3. 通过使用淡隐淡现方式显示效果，显示隐藏的被选元素

   ```javascript
    fadeOut(speed,callback)/fadeIn(speed,callback)  fadeIn 是出来
   ```
