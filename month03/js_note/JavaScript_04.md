...
返回值 : 返回定时器的ID,用于关闭定时器（必须声明下来，因为不声明就关不掉了
并且保存定时器的变量一定是全局的变量，不然后续可能会找不到了）
*/
```
   关闭定时器 :
```javascript
...
1. ##### history
   ...
   作用：保存当前窗口所访问过的URL（不是所有的浏览器的历史记录）
...
2. ##### location
   ...
   作用：保存当前窗口的地址栏信息(URL) 
   属性 :  href 设置或读取当前窗口的地址栏信息 能够通过使用JS的方式实现页面跳转
   方法 :
